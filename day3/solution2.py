...
def calculate_zero_and_one_distribution(data) -> tuple[list[int], list[int]]:
    zeros = []
    ones = []
    for line in data:
...
def increment_bit(index, bit, zeros, ones) -> None:
    try:
        if bit == "0":
...
def filter_data_for_bit(data, bit, index) -> list[str]:
    if len(data) == 1:
        return data
...
def find_data_for_bit_criteria(data, most_common, index=0) -> str:
    if len(data) == 1:
        return data[0]
