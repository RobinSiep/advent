...
def get_lookup_indexes(x: int, y: int) -> tuple[tuple[int, int], ...]:
    return (
        (x - 1, y - 1),
        (x - 1, y),
        (x - 1, y + 1),
...
def contains_negative_index(lookup_indexes: tuple[tuple[int, int], ...]) -> bool:
...
enhanced_image = apply_image_enhancements(wrapped_image, enhancement_algo, 50)
