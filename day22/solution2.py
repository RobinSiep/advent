...
def get_intersection(one: Instruction, other: Instruction) -> Instruction | None:
    _, (x, y, z) = one
    other_state, (other_x, other_y, other_z) = other
    overlap_x = get_overlap(x, other_x)
    overlap_y = get_overlap(y, other_y)
    overlap_z = get_overlap(z, other_z)
    if overlap_x is None or overlap_y is None or overlap_z is None:
        return None
    return (not other_state, (overlap_x, overlap_y, overlap_z))


def get_overlap(one: Boundary, other: Boundary) -> Boundary | None:
