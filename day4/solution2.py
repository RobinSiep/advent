...
class Board:
    ...
    def check_for_winner(self) -> bool | None:
        for row in self.rows:
...
class Game:
    ...
    def __init__(self, boards: list[Board], draws: list[int]) -> None:
        self.boards = boards
        self.draws = draws

    def find_last_winning_score(self) -> int:
        last_winner_score: int | None = None
        ...
        for draw in self.draws:
            ...
            for board in reversed(self.boards):
                if board.check_for_winner():
                    last_winner_score = board.calculate_score(draw)
                    boards.remove(board)
...
def read_board(input_file: TextIO) -> Board | None:
    rows: list[list[Number]] = []
...
print(game.find_last_winning_score())
