...
def read_numbers(filename: str) -> list[SnailfishNumber]:
    numbers = []
...
def add(number_1: SnailfishNumber, number_2: SnailfishNumber) -> SnailfishNumber:
    return reduce([copy.deepcopy(number_1), copy.deepcopy(number_2)])


def reduce(number: SnailfishNumber) -> SnailfishNumber:
    indexes, action_type = find_next_action(number)
    if not action_type or indexes is None:
        return number
...
def find_next_action(
    number: SnailfishNumber | int, indexes: list[int] | None = None
) -> tuple[list[int] | None, Action | None]:
...
def explode(number: SnailfishNumber, indexes: list[int]) -> SnailfishNumber:
...
def get(number: SnailfishNumber | int, indexes: list[int]) -> SnailfishNumber | int:
...
def replace(
    number: SnailfishNumber | int,
    indexes: list[int],
    replacement: SnailfishNumber | int,
) -> None:
    indexes = copy.copy(indexes)
    if len(indexes) == 1:
...
def add_regular_number(
    number: SnailfishNumber | int, indexes: list[int], regular_number: int
) -> None:
    indexes = copy.copy(indexes)
    if len(indexes) == 1:
        number[indexes[0]] = number[indexes[0]] + regular_number
        return

    add_regular_number(number[indexes.pop(0)], indexes, regular_number)


def get_left_explosion_indexes(
    indexes: list[int], number: SnailfishNumber
) -> list[int]:
...
def get_right_explosion_indexes(
    indexes: list[int], number: SnailfishNumber
) -> list[int]:
...
def get_indexes_for_leftmost_regular_number(
    number: SnailfishNumber, indexes: list[int]
) -> list[int]:
...
def split(number: SnailfishNumber, indexes: list[int]) -> SnailfishNumber:
    new_number = copy.copy(number)
...
def find_largest_magnitude(numbers: list[SnailfishNumber]) -> int:
    ...
    for i, number_1 in enumerate(numbers):
        for y, number_2 in enumerate(numbers):
            if i == y:
                continue

            added = add(number_1, number_2)
            magnitude = calculate_magnitude(added)
