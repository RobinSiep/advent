...
from z3 import And, BitVec, BitVecVal, If, Optimize, sat
...
class Program:
    ...
    def execute(self, instruction: tuple[str | int, ...], i: int) -> None:
        function = instruction[0]
        target_name = instruction[1]
        target = self.register[target_name]
        try:
            arg = instruction[2]
    ...
    @contextmanager
    def run(self) -> Generator[Optimize]:
...
def read(filename: str) -> tuple[tuple[str | int, ...], ...]:
    instructions = []
    with open(filename, "r") as input_file:
        for line in input_file:
            instruction: list[str | int] = list(line.strip().split(" "))
...
def find_smallest(instructions: tuple[tuple[str | int, ...], ...]) -> int:
    program = Program()
...
print(find_smallest(instructions))
