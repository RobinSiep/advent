...
MAX_FAILURES_FOR_X_AFTER_SUCCESS = 100
MAX_FAILURES_AFTER_SUCCESS = 1000000
...
def find_boundary(key: str, target_str: str) -> CoordinateRange:
    regex = f"{key}=-?[0-9]*\\.\\.-?[0-9]*"
    low, high = re.findall(regex, target_str)[0][2:].split("..")
    return (int(low), int(high))


def count_distinct_velocities(target_area: Area) -> int:
    x_velocity, y_velocity = (0, -10000)
    velocity_count = 0
    ...
    while True:
        success_for_x = False
        failures_for_x_after_success = 0
        while True:
            ...
            if results == [Result.SUCCESS]:
                ...
                if success_for_x:
                    failures_for_x_after_success += 1

            if failures_after_success >= MAX_FAILURES_AFTER_SUCCESS:
            ...
            if (
                failures_for_x_after_success == MAX_FAILURES_FOR_X_AFTER_SUCCESS
...
distinct_velocity_count = count_distinct_velocities(target_area)
print(distinct_velocity_count)
