...
def get_target_state(initial_state: State) -> State:
    ...
    target_amphipods = [[""] * 4 for i in range(int(len(amphipods) / 4))]
...
def can_leave_room(state: State, room_pos: range) -> int | None:
    for a in room_pos:
        if state[a] == empty:
            continue
        return a
...
def get_possible_valid_pos(from_: int, state: State) -> Iterator[int]:
...
def possible_moves(state: State, target: dict[str, range]) -> Iterator[tuple[int, int]]:
...
def calculate_least_energy(initial_state: State, target_state: State) -> int | None:
